...
my_vfold_cv <- function(data, v = 5, repeats = 1, strata = NULL, breaks = 4,
                     ...) {
  ...
  if (repeats == 1) {
    split_objs <- vfold_splits2(data = data, v = v, strata = strata, 
                               breaks = breaks)
  } else {
    for (i in 1:repeats) {
      tmp <- vfold_splits2(data = data, v = v, strata = strata)
      tmp$id2 <- tmp$id
...
vfold_splits2 <- function(data, v = 10, strata = NULL, breaks = 4) {
  ...
  if (is.null(strata)) {
    ...
    baseline <- c("Disease_4", "Disease_9", "Control_10", "Control_2", "Control_3" )
    # ma_idx <- match(data$replicate, baseline)
    ma_idx <- c(rep(1, 8), rep(2, 8), rep(3, 8), rep(4, 8), rep(5, 8))
    ma_idx[8] <- 4
    ma_idx[16] <- 5
    ma_idx[24] <- 4
    ma_idx[31] <- 1
    ma_idx[32] <- 3
    ma_idx[40] <- 2   
    # ma_idx  <- sample(ma_idx)
    folds <- ifelse(!is.na(ma_idx), ma_idx, data$replicate)
    idx <- seq_len(n)
